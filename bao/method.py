...
def shaixuan(html):  # 对源码进行筛选
    urls = re.findall(
        '<div id=".*?" data-id="(.*?)" class="thumb-block ">',
        html,
        re.S)
    return urls
...
def get_photo(entry, entry1, entry2, entry3, str4, combobox, combobox1):
    if entry.get() != '' and entry1.get() != '' and entry2.get(
    ) != '' and entry3.get() != '' and str4.get() != '' and combobox.get() != '':
        top = Toplevel()  # 创建弹出式窗体
        top.title('爬取页面')
        top.geometry("350x580+360+200")
        top.resizable(0, 0)
        k = os.path.abspath(os.path.join(os.getcwd(), '..'))
        decodepicture(k + '/image/b', b.img, '.ico')
        top.iconbitmap(k + '/image/b.ico')
        huabu = tkinter.Canvas(top, width=350, height=580)
        decodepicture(k + '/image/d', s.img, '.jpg')
        tupian = get_image(k + '/image/d.jpg', 350, 580)
        huabu.create_image(175, 290, image=tupian)
        huabu.pack()
        str1 = StringVar()
        str2 = StringVar()
        str3 = StringVar()
            ...
            for page in pages:
                try:
                    ...
                    if combobox1.get() == "高":
                        ...
                        url = re.findall(
                            r'html5player.setVideoUrlLow\(\'(.*?)\'\);', html, re.S)[0]
                    ...
                    response = requests.get(
                        url, stream=True, headers=headers, timeout=5)
...
def guanyu():
    tkinter.messagebox.showinfo('提示', '该软件由WBB开发，大家可以放心使用！！！')
