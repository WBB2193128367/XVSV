...
root.geometry("350x580+750+200")
k = os.path.abspath(os.path.join(os.getcwd(), '..'))
method.decodepicture(k + '/image/a', a.img, '.ico')
root.iconbitmap(k + '/image/a.ico')
root.resizable(0, 0)
huabu = tkinter.Canvas(root, width=350, height=580)
method.decodepicture(k + '/image/e', e.img, '.jpg')
tupian = method.get_image(k + '/image/e.jpg', 350, 560)

# method.decodepicture(k+'/image/f', l.img, '.jpg')
# tupian1=method.get_image(k + '/image/f.jpg', 350, 540)

huabu.create_image(175, 280, image=tupian)
huabu.pack()
...
lb = Label(root, text="请选择系统或自定义类型:  ", state='active')
lb.place(x=25, y=150)
combobox = ttk.Combobox(root, width=17, state='readonly')
combobox['values'] = ("系统类型", "自定义类型")
combobox.current(1)
combobox.place(x=180, y=150)
lb = Label(root, text="请输入要爬取的视频类型:  ", state='active')
lb.place(x=25, y=185)
entry = Entry(root)
entry.place(x=180, y=185)
lb = Label(root, text="请输入视频命名开始的序号:", state='active')
lb.place(x=25, y=220)
entry1 = Entry(root)
entry1.place(x=180, y=220)
lb = Label(root, text="请输入开始爬取的网页号:  ", state='active')
lb.place(x=25, y=255)
entry2 = Entry(root)
...
btn = Button(
    ...
    command=lambda: method.get_photo(
        entry,
        entry1,
        entry2,
        entry3,
